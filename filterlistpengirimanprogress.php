...
$kueri_waiting = mysqli_query($con, "SELECT * FROM tbl_report as r left join tbl_product_catalog as pc on r.id_product_catalog = pc.id_product_catalog WHERE product_name like '%$keyword%' and status='progress' LIMIT $posisi,$batas");
$row = mysqli_num_rows($kueri_waiting);
?>
<form id="form1" name="form1" method="post" action="admin.php?page=pengiriman&amp;&amp;status=filterprogress">
  Search by Product
...
<table width="100%" class="table table-responsive table-hover">
  ...
  <tr>
			...
			<div class="modal-dialog">
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<?php
						if($data_modal['received_date']=="0000-00-00")
						{
						?>
						<tr>
							<td>Received Date</td>
							<td width="5px" align="center">:</td>
							<td width="100px">
							<form method="post" id="add_received_date" class="form-horizontal" action="prosesaddreceiveddate.php">
							<input name="received_date<?=$data_waiting['id_report'];?>" class="text" id="received_date<?=$data_waiting['id_report'];?>" size="10" maxlength="10">
						...
						<script language="javascript" type="text/javascript">
						 $(document).ready(function(){
							$(function() {	
								$( '#received_date<?=$data_waiting['id_report'];?>' ).datepicker({
									dateFormat:'yy-mm-dd',
									changeMonth: true,
									changeYear: true
								});		
    ...
    <br />
    <a href="deletereport.php?status=progress&&id=<?=$data_waiting['id_report'];?>" onClick="return confirm('Are you sure you want to DELETE??');"><div class="icon-trash"></div></a>
    </td>
    <?php
	}
	?>
...
<div align="center">
    ...
    $tampil2="SELECT * FROM tbl_report as r left join tbl_product_catalog as pc on r.id_product_catalog = pc.id_product_catalog WHERE product_name like '%$keyword%' and status='progress'";
    ...
    {
        $previous=$halaman-1;
        echo "<A HREF=$file?page=pengiriman&&status=filterprogress&&halaman=1&&keyword=$keyword><< First</A> | 
            <A HREF=$file?page=pengiriman&&status=filterprogress&&halaman=$previous&&keyword=$keyword>< Previous</A> | ";
    }
    else
    { 
        echo "<< First | < Previous | ";
    ...
    for($i=$halaman-2;$i<$halaman;$i++)
    {
      if ($i < 1) 
          continue;
      $angka .= "<a href=$file?page=pengiriman&&status=filterprogress&&halaman=$i&&keyword=$keyword>$i</A> ";
    }
    ...
    $angka .= ($halaman+2<$jmlhalaman ? " ...  
              <a href=$file?page=pengiriman&&status=filterprogress&&halaman=$jmlhalaman&&keyword=$keyword>$jmlhalaman</A> " : " ");
    ...
    {
        $next=$halaman+1;
        echo " | <A HREF=$file?page=pengiriman&&status=filterprogress&&halaman=$next&&keyword=$keyword>Next ></A> | 
      <A HREF=$file?page=pengiriman&&status=filterprogress&&halaman=$jmlhalaman&&keyword=$keyword>Last >></A> ";
    }
    else
    { 
        echo " | Next > | Last >>";
