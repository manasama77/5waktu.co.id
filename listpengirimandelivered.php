...
?>
<?php
$kueri_waiting = mysqli_query($con, "SELECT * FROM tbl_report WHERE status='delivered' LIMIT $posisi,$batas");
...
<div align="center">
    ...
    $tampil2="select * from tbl_report where status='delivered'";
    ...
    {
        $previous=$halaman-1;
        echo "<A HREF=$file?page=pengiriman&&halaman=1&&status=delivered><< First</A> | 
            <A HREF=$file?page=pengiriman&&halaman=$previous&&status=delivered>< Previous</A> | ";
    }
    else
    { 
        echo "<< First | < Previous | ";
    ...
    for($i=$halaman-2;$i<$halaman;$i++)
    {
      if ($i < 1) 
          continue;
      $angka .= "<a href=$file?page=pengiriman&&halaman=$i&&status=delivered>$i</A> ";
    }
    ...
    $angka .= ($halaman+2<$jmlhalaman ? " ...  
              <a href=$file?page=pengiriman&&halaman=$jmlhalaman&&status=delivered>$jmlhalaman</A> " : " ");
    ...
    {
        $next=$halaman+1;
        echo " | <A HREF=$file?page=pengiriman&&halaman=$next&&status=delivered>Next ></A> | 
      <A HREF=$file?page=pengiriman&&halaman=$jmlhalaman&&status=delivered>Last >></A> ";
    }
    else
    { 
        echo " | Next > | Last >>";
