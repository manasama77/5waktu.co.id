...
?>
<?php
$kueri_waiting = mysqli_query($con, "SELECT * FROM tbl_report WHERE status='progress' LIMIT $posisi,$batas");
...
<div align="center">
    ...
    {
        $previous=$halaman-1;
        echo "<A HREF=$file?page=pengiriman&&halaman=1&&status=progress><< First</A> | 
            <A HREF=$file?page=pengiriman&&halaman=$previous&&status=progress>< Previous</A> | ";
    }
    else
    { 
        echo "<< First | < Previous | ";
    ...
    for($i=$halaman-2;$i<$halaman;$i++)
    {
      if ($i < 1) 
          continue;
      $angka .= "<a href=$file?page=pengiriman&&halaman=$i&&status=progress>$i</A> ";
    }
    ...
    $angka .= ($halaman+2<$jmlhalaman ? " ...  
              <a href=$file?page=pengiriman&&halaman=$jmlhalaman&&status=progress>$jmlhalaman</A> " : " ");
    ...
    {
        $next=$halaman+1;
        echo " | <A HREF=$file?page=pengiriman&&halaman=$next&&status=progress>Next ></A> | 
      <A HREF=$file?page=pengiriman&&halaman=$jmlhalaman&&status=progress>Last >></A> ";
    }
    else
    { 
        echo " | Next > | Last >>";
