...
<?php
include("config.php");
$excel_start=$_REQUEST['excel_start1'];
$excel_end=$_REQUEST['excel_end1'];
...
AND pengiriman.status_pengiriman = 'progress'");
...
<div class="box-body">
	...
	<table id="mytable" name="mytable" class="table table-bordered table-striped small">
		<thead>
			<tr>
				<th style="text-align:center;" colspan="27">Progress Data - <?=$excel_start;?> ~ <?=$excel_end;?></th>
			<tr>
			<tr>
		...
		$kueri_sum_harga=mysqli_query($con,"SELECT SUM(tbl_pengiriman.sum_harga) AS sum_total_harga FROM tbl_pengiriman WHERE tbl_pengiriman.exit_date BETWEEN '$excel_start' AND '$excel_end' AND tbl_pengiriman.status_pengiriman = 'progress'");
...
function exportexcel() {
	$("#mytable").table2excel({
		name: "Export Progress Data <?=$excel_start;?> - <?=$excel_end;?>",
		filename: "Export Progress Data <?=$excel_start;?> - <?=$excel_end;?>.xls",
		fileext: ".xls"
	});
}
</script>