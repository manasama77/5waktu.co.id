...
if ($id_dealer != '') {
	...
	$qdeal = "WHERE p.id_dealer = '$id_dealer'";
	$abbr_where = "AND";
} else {
	$qdeal = "";
	$abbr_where = "WHERE";
...
$sql = "SELECT * FROM tbl_pengiriman AS p LEFT JOIN tbl_kota AS kota ON p.id_kota = kota.id_kota LEFT JOIN tbl_dealer AS dealer ON p.id_dealer = dealer.id_dealer $qdeal ";
...
// $sql = "SELECT * FROM tbl_pengiriman AS p LEFT JOIN tbl_kota AS kota ON p.id_kota = kota.id_kota LEFT JOIN tbl_dealer AS dealer ON p.id_dealer = dealer.id_dealer WHERE YEAR(p.exit_date) = '$tahun_sekarang' $qdeal ";
$sql = "SELECT COUNT(*) FROM tbl_pengiriman AS p LEFT JOIN tbl_kota AS kota ON p.id_kota = kota.id_kota LEFT JOIN tbl_dealer AS dealer ON p.id_dealer = dealer.id_dealer $qdeal ";

if (!empty($requestData['search']['value'])) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
	$sql .= " $abbr_where ( p.do_num LIKE '%" . $requestData['search']['value'] . "%' ";
	$sql .= " OR MATCH (dealer.nama_dealer) AGAINST ('" . $requestData['search']['value'] . "*' IN BOOLEAN MODE) ";
...
while ($row = mysqli_fetch_array($query)) {  // preparing an array
	...
	$nestedData[] = '<button type="button" class="btn btn-info btn-sm" onClick="loaddashboardmodal(' . $row['id_pengiriman'] . ')">' . $row['do_num'] . '</button>';
