...
<?php
$keyword_vehicle = $_REQUEST['keyword_vehicle'];
...
{
	$kueri_mobil = mysqli_query($con, "SELECT mobil.id_vehicle, mobil.no_polisi, mobil.merk, mobil.jenis, mobil.jumlah_ban, mobil.isi_silinder, mobil.tahun_pembuatan, mobil.no_rangka, mobil.no_mesin, mobil.period_stnk, mobil.bahan_bakar, mobil.warna, driver.nama FROM tbl_vehicle AS mobil LEFT JOIN tbl_driver AS driver ON mobil.id_driver = driver.id_driver WHERE no_polisi LIKE '%$keyword_vehicle%' ORDER BY mobil.id_vehicle LIMIT $posisi,$batas");
}
...
<div align="center">
    ...
    $tampil2="select * from tbl_vehicle WHERE no_polisi LIKE '%$keyword_vehicle%'";
    ...
    {
        $previous=$halaman-1;
        echo "<A HREF=$file?page=configuration&&tab=fvehicle&&halaman=1&&keyword_vehicle=$keyword_vehicle><< First</A> | 
            <A HREF=$file?page=configuration&&tab=fvehicle&&halaman=$previous&&keyword_vehicle=$keyword_vehicle>< Previous</A> | ";
    }
    else
    { 
        echo "<< First | < Previous | ";
    ...
    for($i=$halaman-2;$i<$halaman;$i++)
    {
      if ($i < 1) 
          continue;
      $angka .= "<a href=$file?page=configuration&&tab=fvehicle&&halaman=$i&&keyword_vehicle=$keyword_vehicle>$i</A> ";
    }
    ...
    $angka .= ($halaman+2<$jmlhalaman ? " ...  
              <a href=$file?page=configuration&&tab=fvehicle&&halaman=$jmlhalaman&&keyword_vehicle=$keyword_vehicle>$jmlhalaman</A> " : " ");
    ...
    {
        $next=$halaman+1;
        echo " | <A HREF=$file?page=configuration&&tab=fvehicle&&halaman=$next&&keyword_vehicle=$keyword_vehicle>Next ></A> | 
      <A HREF=$file?page=configuration&&tab=fvehicle&&halaman=$jmlhalaman&&keyword_vehicle=$keyword_vehicle>Last >></A> ";
    }
    else
    { 
        echo " | Next > | Last >>";
