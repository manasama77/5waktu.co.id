...
if(isset($_GET['pencarian']))
{
	...
	$kueri_waiting = mysqli_query($con, "SELECT * FROM tbl_satuan_harga_kota AS shk LEFT JOIN tbl_kota AS kota ON shk.id_kota = kota.id_kota LEFT JOIN tbl_kategori_produk AS kp ON shk.id_kategori_produk = kp.id_kategori_produk WHERE kota.nama_kota LIKE '%$keyword%' ORDER BY kota.nama_kota ASC LIMIT $posisi,$batas");
}
else
{
	$kueri_waiting = mysqli_query($con, "SELECT * FROM tbl_satuan_harga_kota AS shk LEFT JOIN tbl_kota AS kota ON shk.id_kota = kota.id_kota LEFT JOIN tbl_kategori_produk AS kp ON shk.id_kategori_produk = kp.id_kategori_produk ORDER BY kota.nama_kota ASC LIMIT $posisi,$batas");
}

$row = mysqli_num_rows($kueri_waiting);
?>
<div class="container">
	<div class="widget-header"><i class="fa fa-list-alt"></i><h3>Setting - <font color="blue">Satuan Harga Dasar</font></h3>
</div>
<div class="widget-content">
	...
	<tr>
		<td align="left">
			...
			<img src="img/Button-Add-icon.png" width="30px" style="padding:5px;" /> Add Satuan Harga Dasar</a></h3>
		</td>
	</tr>
	</table>
	<?php
	...
	<table width="100%" class="table table-responsive table-hover">
	  ...
	  <tr>
		...
		<td align="center"><?=number_format($data_waiting['satuan_harga']);?></td>
		<?php
		if($_SESSION['login']['level'] == "super_admin"){
		?>
			<td align="center">
			<a href="admin.php?page=editsatuanhargakota&&id=<?=$data_waiting['id_satuan_harga_kota'];?>">				<button type="button" class="btn btn-primary btn-sm" title="Edit Satuan Harga Dasar"><div class="fa fa-pencil"></div></button>			</a>
			<a href="deletesatuanhargakota.php?id=<?=$data_waiting['id_satuan_harga_kota'];?>" onClick="return confirm('Are you sure you want to DELETE??');">				<button type="button" class="btn btn-danger btn-sm" title="Hapus Satuan Harga Dasar"><div class="fa fa-trash"></div></button>			</a>
			</td>
		<?php
		}
		?>
