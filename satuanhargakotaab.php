...
?>
<div class="container">
	<div class="widget-header"><i class="fa fa-database fa-2x"></i><h3>Setting - <font color="blue">Satuan Harga Class A & B</font></h3>
</div>
<div class="widget-content">
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		<td align="left">
			<h3><a href="admin.php?page=addsatuanhargakotaab">
			<img src="img/Button-Add-icon.png" width="30px" style="padding:5px;" /> Add Satuan Harga kota A & B</a></h3>
		</td>
	</tr>
	</table>
	<form id="form1" name="form1" method="post" action="admin.php?page=satuanhargakotaab&&pencarian=on">
		Cari Kota
	...
	<table width="100%" class="table table-responsive table-hover">
	  ...
	  <tr>
		...
		?>
			<td align="center">
			<a href="admin.php?page=editsatuanhargakotaab&&id=<?=$data_waiting['id_satuan_harga_kota'];?>"><div class="fa fa-edit"></div></a>
			<br />
			<a href="deletesatuanhargakotaab.php?id=<?=$data_waiting['id_satuan_harga_kota'];?>" onClick="return confirm('Are you sure you want to DELETE??');"><div class="fa fa-trash"></div></a>
			</td>
		<?php
		}
		?>
	...
	<div align="center">
		...
		if(isset($_GET['pencarian']))
		{
			...
			{
				$previous=$halaman-1;
				echo "<A HREF=$file?page=satuanhargakotaab&&halaman=1&&pencarian=on&&keyword=$keyword><< First</A> | 
					<A HREF=$file?page=satuanhargakotaab&&halaman=$previous&&pencarian=on&&keyword=$keyword>< Previous</A> | ";
			}
			else
			{ 
				echo "<< First | < Previous | ";
			...
			for($i=$halaman-2;$i<$halaman;$i++)
			{
			  if ($i < 1) 
				  continue;
			  $angka .= "<a href=$file?page=satuanhargakotaab&&halaman=$i&&pencarian=on&&keyword=$keyword>$i</A> ";
			}
			...
			$angka .= ($halaman+2<$jmlhalaman ? " ...  
					  <a href=$file?page=satuanhargakotaab&&halaman=$jmlhalaman&&pencarian=on&&keyword=$keyword>$jmlhalaman</A> " : " ");
			...
			{
				$next=$halaman+1;
				echo " | <A HREF=$file?page=satuanhargakotaab&&halaman=$next&&pencarian=on&&keyword=$keyword>Next ></A> | 
			  <A HREF=$file?page=satuanhargakotaab&&halaman=$jmlhalaman&&pencarian=on&&keyword=$keyword>Last >></A> ";
			}
			else
			{ 
				echo " | Next > | Last >>";
		...
		else
		{
			...
			{
				$previous=$halaman-1;
				echo "<A HREF=$file?page=satuanhargakotaab&&halaman=1><< First</A> | 
					<A HREF=$file?page=satuanhargakotaab&&halaman=$previous>< Previous</A> | ";
			}
			else
			{ 
				echo "<< First | < Previous | ";
			...
			for($i=$halaman-2;$i<$halaman;$i++)
			{
			  if ($i < 1) 
				  continue;
			  $angka .= "<a href=$file?page=satuanhargakotaab&&halaman=$i>$i</A> ";
			}
			...
			$angka .= ($halaman+2<$jmlhalaman ? " ...  
					  <a href=$file?page=satuanhargakotaab&&halaman=$jmlhalaman>$jmlhalaman</A> " : " ");
			...
			{
				$next=$halaman+1;
				echo " | <A HREF=$file?page=satuanhargakotaab&&halaman=$next>Next ></A> | 
			  <A HREF=$file?page=satuanhargakotaab&&halaman=$jmlhalaman>Last >></A> ";
			}
			else
			{ 
				echo " | Next > | Last >>";
