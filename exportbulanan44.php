...
WHERE MONTH(pengiriman.exit_date) = '$excel_bulanan' AND YEAR(pengiriman.exit_date) = '$excel_tahun'");
...
<div class="box-body">
	...
	<table id="mytable" name="mytable" class="table table-bordered table-striped small">
		<thead>
			<tr>
				<th style="text-align:center;" colspan="27">Monthly Data - <?=$monthName;?> <?=$excel_tahun;?></th>
			<tr>
			<tr>
		...
		$kueri_sum_harga=mysqli_query($con,"SELECT SUM(tbl_pengiriman.sum_harga) AS sum_total_harga FROM tbl_pengiriman WHERE MONTH(tbl_pengiriman.exit_date) = '$excel_bulanan' AND YEAR(tbl_pengiriman.exit_date) = '$excel_tahun'");
...
function exportexcel() {
	$("#mytable").table2excel({
		name: "Export Bulanan Data <?=$monthName;?> <?=$excel_tahun;?>",
		filename: "Export Bulanan Data <?=$monthName;?> <?=$excel_tahun;?>.xls",
		fileext: ".xls"
	});
}
</script>
