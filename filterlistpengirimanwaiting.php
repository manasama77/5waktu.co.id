...
$kueri_waiting = mysqli_query($con, "SELECT * FROM tbl_report as r left join tbl_product_catalog as pc on r.id_product_catalog = pc.id_product_catalog WHERE product_name like '%$keyword%' and status='waiting' LIMIT $posisi,$batas");
$row = mysqli_num_rows($kueri_waiting);
?>  
<form id="form1" name="form1" method="post" action="admin.php?page=pengiriman&amp;&amp;status=filterwaiting">
  Search by Product
...
<table width="100%" class="table table-responsive table-hover">
  ...
  <tr>
			...
			<div class="modal-dialog">
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<?php
						if($data_modal['do_print_date']=="0000-00-00")
						{
						?>
						<tr>
							<td>DO Print Date</td>
							<td width="5px" align="center">:</td>
							<td width="100px">
							<form method="post" id="add_do_print_date" class="form-horizontal" action="prosesadddoprintdate.php">
							<input name="do_print_date<?=$data_waiting['id_report'];?>" class="text" id="do_print_date<?=$data_waiting['id_report'];?>" size="10" maxlength="10">
							<input name="id_report" type="hidden" id="id_report" value="<?=$data_waiting['id_report'];?>" />
						...
						<script language="javascript" type="text/javascript">
						 $(document).ready(function(){
							$(function() {	
								$( '#do_print_date<?=$data_waiting['id_report'];?>' ).datepicker({
									dateFormat:'yy-mm-dd',
									changeMonth: true,
									changeYear: true
								});		
						...
						?>
						<?php
						if($data_modal['exit_date']=="0000-00-00")
						{
						?>
						<tr>
							<td>Exit Date</td>
							<td width="5px" align="center">:</td>
							<td width="100px">
							<form method="post" id="add_exit_date" class="form-horizontal" action="prosesaddexitdate.php">
							<input name="exit_date<?=$data_waiting['id_report'];?>" class="text" id="exit_date<?=$data_waiting['id_report'];?>" size="10" maxlength="10">
							<input name="id_report" type="hidden" id="id_report" value="<?=$data_waiting['id_report'];?>" />
							<input name="duration" type="hidden" id="duration" value="<?=$data_modal['duration'];?>" />
							<button class="btn btn-primary" type="submit">Save</button>
							</form>
							</td>
						</tr>
						<script language="javascript" type="text/javascript">
						 $(document).ready(function(){
							$(function() {	
								$( '#exit_date<?=$data_waiting['id_report'];?>' ).datepicker({
									dateFormat:'yy-mm-dd',
									changeMonth: true,
									changeYear: true
								});		
    ...
    <br />
    <a href="deletereport.php?status=waiting&&id=<?=$data_waiting['id_report'];?>" onClick="return confirm('Are you sure you want to DELETE??');"><div class="icon-trash"></div></a>
    </td>
    <?php
	}
	?>
...
<div align="center">
    ...
    $tampil2="SELECT * FROM tbl_report as r left join tbl_product_catalog as pc on r.id_product_catalog = pc.id_product_catalog WHERE product_name like '%$keyword%' and status='waiting'";
    ...
    {
        $previous=$halaman-1;
        echo "<A HREF=$file?page=pengiriman&&status=filterwaiting&&halaman=1&&keyword=$keyword><< First</A> | 
            <A HREF=$file?page=pengiriman&&status=filterwaiting&&halaman=$previous&&keyword=$keyword>< Previous</A> | ";
    }
    else
    { 
        echo "<< First | < Previous | ";
    ...
    for($i=$halaman-2;$i<$halaman;$i++)
    {
      if ($i < 1) 
          continue;
      $angka .= "<a href=$file?page=pengiriman&&status=filterwaiting&&halaman=$i&&keyword=$keyword>$i</A> ";
    }
    ...
    $angka .= ($halaman+2<$jmlhalaman ? " ...  
              <a href=$file?page=pengiriman&&status=filterwaiting&&halaman=$jmlhalaman&&keyword=$keyword>$jmlhalaman</A> " : " ");
    ...
    {
        $next=$halaman+1;
        echo " | <A HREF=$file?page=pengiriman&&status=filterwaiting&&halaman=$next&&keyword=$keyword>Next ></A> | 
      <A HREF=$file?page=pengiriman&&status=filterwaiting&&halaman=$jmlhalaman&&keyword=$keyword>Last >></A> ";
    }
    else
    { 
        echo " | Next > | Last >>";
