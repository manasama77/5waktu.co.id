...
<div align="center">
    ...
    $tampil2="select * from tbl_product_catalog where product_name like '%$keyword%'";
    ...
    {
        $previous=$halaman-1;
        echo "<A HREF=$file?page=product&&halaman=1&&filter&&keyword=$keyword><< First</A> | 
            <A HREF=$file?page=product&&halaman=$previous&&filter&&keyword=$keyword>< Previous</A> | ";
    }
    else
    { 
        echo "<< First | < Previous | ";
    ...
    for($i=$halaman-2;$i<$halaman;$i++)
    {
      if ($i < 1) 
          continue;
      $angka .= "<a href=$file?page=product&&halaman=$i&&filter&&keyword=$keyword>$i</A> ";
    }
    ...
    $angka .= ($halaman+2<$jmlhalaman ? " ...  
              <a href=$file?page=product&&halaman=$jmlhalaman&&filter&&keyword=$keyword>$jmlhalaman</A> " : " ");
    ...
    {
        $next=$halaman+1;
        echo " | <A HREF=$file?page=product&&halaman=$next&&filter&&keyword=$keyword>Next ></A> | 
      <A HREF=$file?page=product&&halaman=$jmlhalaman&&filter&&keyword=$keyword>Last >></A> ";
    }
    else
    { 
        echo " | Next > | Last >>";
